...
export function renderHome(): string {
  return [
    ...
    ui.table([
      ...
      ["hubolt worker start", "Run the webhook review worker (requires Redis)"],
      ["hubolt webhooks verify-fixture", "Validate webhook signature and payload fixtures"],
      ["hubolt github post", "Post a review report to a GitHub pull request"],
      ["hubolt github map-lines", "Debug diff line mapping for a report"],
      ["hubolt history", "Show review history stored on the server"],
      ["hubolt history --trends", "Trend metrics: volume, acceptance, top rules"],
      ["hubolt gateway test", "Verify gateway health, budgets, and audit logging"],
      ["hubolt audit export", "Export server audit events to a file"],
      ["hubolt feedback import", "Import finding feedback from a PR or file"],
      ["hubolt memory list", "Show team memory cards"],
      ["hubolt memory inspect", "Explain which cards a review would retrieve"],
      ["hubolt memory rebuild", "Regenerate memory cards from feedback"],
    ]),
    "",
